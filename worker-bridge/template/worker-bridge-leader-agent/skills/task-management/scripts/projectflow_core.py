"""Local projectflow state machine for the worker-bridge AgentTeams leader.

Vendored from ``AgentTeams/copaw/src/copaw_worker/task.py`` (dev-v1.2.2) —
the COMPLETE original, worker and leader surface together, so the trio
(projectflow.py + projectflow_core.py + mc_sync.py) deploys into a leader
skill's scripts/ directory standalone, exactly like the taskflow trio.
The CLI layer (projectflow.py) exposes only the leader commands; the
worker functions stay here so this core stays line-equivalent to the
single copaw source of truth for both roles.

Kept line-equivalent: ProjectMeta/DagTask/LoopPlan/TaskMeta/TaskResult,
FileSystemTaskStore (project + task files), create/add/plan/ready/loop/
delegate/commit/pause/resume/complete/check, ack/submit, DAG + Loop
parse/render/replace/validate, result.md parse/render/validate, and
identity normalization — so protocol output (plan.md, meta.json,
spec.md, result.md) stays byte-compatible with copaw.

Deliberate deviation (the only one, same as taskflow_core.py): every file
read/write passes ``encoding="utf-8"`` explicitly (the original relies on
the platform default, which is UTF-8 in the Linux sandbox but GBK on the
Windows dev machines), and every write passes ``newline="\n"`` so
protocol files carry LF line endings on every platform — identical to
what the Linux copaw original produces.
"""
