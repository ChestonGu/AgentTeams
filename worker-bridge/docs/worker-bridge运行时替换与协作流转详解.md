...
> **统一命名注记（2026-09-13，v3 分支规整）**：runtime 统一为 **worker-bridge**（原 opencode），
> opencode 链路全量退役。新链路：CR 5 字段（adapterMode/cimicodeGatewayUrl/sessionId/sandboxId/templateId）
> → controller 投影 runtime.yaml 顶层 bridge 段 → bridge 按 §3.3 裁决（显式 env > bridge 段 > 未定自愈）；
...
## 十、生产迁移：组件分层与 cimicode 沙箱预置

> 目标架构：bridge → **无状态 cimicode**（接口调用），cimicode 使用过程中**自建沙箱**（team 侧零管控）。结论先行：opencode/sandbox 双 pod、opencode-stack-operator、钉 105 全部是测试专用物，**不需要迁移**；要带走的是 bridge 本体、controller 少量通用修复、协作工具/skill 预置层（已抽成 [opencode/cimicode-sandbox/Dockerfile](../cimicode-sandbox/Dockerfile)）。

### 10.1 分层清单：哪些直接用、哪些不能带

**A. 直接可用（生产迁移核心资产）**

| 资产 | 说明 |
|---|---|
| **cimicode-bridge 本体**（v0.1.9） | 与具体运行时解耦：matrix 防护、turn 编排、每 turn 现拉+agent.md 生成、事件回流。对接新运行时只需一个 adapter（§10.4） |
...
| **协作工具与 skills 预置层** | `opencode/cimicode-sandbox/Dockerfile`：taskflow/agentteams-sync CLI + 5 worker skills + mc/jq/git/python3，零 opencode 依赖 |
| **controller 通用修复**（v1.2.2.5/6） | pod-template env hybrid、建 pod 前就绪屏障——与运行时无关的健壮性修复 |
| **协议约定** | 结构化 mention、taskflow 状态机、MinIO 目录布局、rc_invites 放宽、`.gitattributes *.sh eol=lf`、资源三层限额思路 |
...
| **bash.ts、ripgrep、skill 等待 entrypoint** | opencode 专属（skill 工具依赖、启动扫描竞态） |
...
| 资产 | 适配点 |
|---|---|
| bridge 的 **runtime adapter 层** | 现有 opencode/cimicode(SSE) 两实现；为无状态 cimicode 写第三个（§10.4） |
| **agent.md 注入方式** | opencode 靠"共享卷 AGENTS.md 文件"生效；cimicode 若接口直收 system prompt 则跳过文件落盘，`chat(agent_md=...)` 直传 |
| **worker pod 形态** | 现在 bridge 以 worker 伪装跑成 pod（controller 建）；若 cimicode 是平台侧服务，bridge 部署形态随之调整，controller 的 masquerade 镜像 env（`AGENTTEAMS_OPENCODE_BRIDGE_IMAGE`）换成 cimicode 对接约定 |
| MinIO/OSS 端点与凭据来源 | 测试用 chart 内 MinIO+root 凭据；生产按真实对象存储+per-worker 凭据（契约不变：`AGENTTEAMS_FS_*` 四件套+TEAM+MATRIX_USER_ID） |

### 10.2 controller 为 bridge 做了什么（迁移对照）

1. **worker 伪装建 pod**：Worker CR 的 runtime 留空 → `AGENTTEAMS_DEFAULT_WORKER_RUNTIME` env 兜底解析 → controller 建 worker pod 时用 `AGENTTEAMS_OPENCODE_BRIDGE_IMAGE` 指的 bridge 镜像（`kubernetes.go OpenCodeBridgeImage`）。bridge pod 因此免费获得全部 worker 基础设施：CR finalizer 生命周期、SA projected token、env 注入、Matrix 账号供给、个人房间。
...
3. **就绪屏障**（v1.2.2.6）：`member_reconcile` 在 `wb.Create` 前，managed runtime 调 `Deployer.RuntimeConfigReadyForBootstrap`（读 runtime.yaml，`member.matrixUserId` 非空才放行，否则 5s requeue）。保证 bridge pod 诞生时刻 bootstrap 必然一次成型。**与运行时无关，生产保留**。
4. **env hybrid**（v1.2.2.5）：pod-template ConfigMap 的 worker 容器 env 先落、controller 同名覆盖——运维一处配置全员生效（测试用它注入 `COPAW_TOOL_GUARD_ENABLED=false`）。注意**线上 ConfigMap 有 env 段而仓库副本 `opencode/deploy/pod-template-oct.yaml` 没有**，迁移时从线上抄回。
5. controller **不感知 opencode 细节**：只做"managed runtime 的通用供给"。替换成 cimicode 后 1/4/5 原样适用；2 的 runtime.yaml 机制原样适用（cimicode 的 agent.md 同样从它渲染）。

### 10.3 opencode+sandbox 的"自动"供给（operator 细节，仅供理解现有测试链）
...
共享卷：测试用同节点 hostPath `/workspace`（钉 105 的原因之一）；生产若保留双 pod 形态应换 RWX PVC，目标架构（cimicode 单体自管沙箱）没有这个问题。

### 10.4 bridge 对接无状态 cimicode（目标架构的接缝）
...
1. **system prompt**：`chat(agent_md=...)` 每 turn 传入现生成的 agent.md——cimicode 接口若直收 system prompt 直接用；若只认文件，则由 cimicode 自己落盘（bridge 不再需要 helper URL）。
...
3. **长 turn**：POST 阻塞或轮询均可，超时用 `BRIDGE_RUNTIME_TURN_TIMEOUT`（默认 3600s）；**务必实现"超时≠失败"语义**（opencode adapter 的文案范式："task still running server-side; ask again to re-attach"）。
4. **中间进度**：turn 结束把多条 assistant 文本拆 progress_texts，bridge 逐条发房间（协作可见性的关键）。
5. 现成参照：`runtime/opencode_adapter.py`（REST+轮询收尾）与 `runtime/adapters.py` 的 cimicode SSE 实现。

### 10.5 cimicode 沙箱预置（team 零管控）

只需两件事：

1. **基础镜像** = [opencode/cimicode-sandbox/Dockerfile](../cimicode-sandbox/Dockerfile)，从现 sandbox 镜像裁剪：
   - **预置**：5 个 worker skills 全文（communication / file-sharing / mcporter / organization / task-management，SKILL.md+scripts）→ `/opt/agentteams/skills/`；协作 CLI 到 PATH（`taskflow`、`agentteams-sync`）；运行依赖 python3.12 / mc / jq / git / procps。leader 场景追加 leader 模板的 project-management / team-coordination 两件（Dockerfile 注释标明）。
   - **裁剪掉**：sandbox_helper(:4097)、opencode、bash.ts、ripgrep、entrypoint 守护（启动命令 cimicode 自己定）。
...
1. bridge 对接：写 cimicode adapter（§10.4）+ 单测（FakeRuntime 框架现成）
2. cimicode 沙箱：用新 Dockerfile 出镜像，cimicode 建沙箱流程注入 env 契约，验证 `taskflow ack/submit` 走通对象存储
