...
- `cimicode-pod`：`operator/`（runtime 无关供给器）供给单 runtime pod——
  Deployment+svc（单端口 :4096）+ Secret（FS 凭据 + model-config），模型经
  `OPENCODE_CONFIG_CONTENT` secretKeyRef 注入（镜像零凭据，spec.model +
  bridge pod 网关 env 渲染），并 patch Worker CR env
  `BRIDGE_RUNTIME_ADAPTER`/`_BASE_URL` 两键，bridge 自愈轮询接上。runtime
  镜像双形态同契约：`cimicode-runtime/`（内部 coder-cimicode 基础镜像）/
  `opencode-runtime/`（外网 npm opencode-ai@1.18.27），切换只换镜像。
  传输契约见 [contract/adapter-contract.md](contract/adapter-contract.md)。
...
| `contract/` | `interface-contract.md` v2.4（§0 架构决策 / env / 镜像布局 / shared 唯一同步 / 消息 / 命令（worker §5.1-5.3 + leader §5.4）/ **统一日志 §5.5** / agent.md **生成契约 §6**（源模板 + runtime.yaml + persona）/ 职责矩阵）+ `controller-handover.md` v2.4（零代码改动 + qwenpaw/edge 分支前提）+ `adapter-contract.md` v1.1（统一形态传输契约：stateless SSE / pod 单端口 REST+轮询+消息体 system 字段、双 runtime 镜像、模型注入、两键接线、已知限制） | T8-T9 ✅（v2 重写；adapter 契约 2026-09-13 增补，v1.1 统一形态 2026-09-16） |
| `operator/` | **worker-bridge-operator**（runtime 无关供给器）：watch runtime=worker-bridge 的 Worker CR，按 `spec.adapterMode` 分派——`cimicode-stateless` 零供给；`cimicode-pod`（含空值）供给单 runtime Deployment+svc（命名 `<w>-cimicode` / `<w>-cimicode-svc`，角色后缀式；单端口 :4096，`CIMICODE_IMAGE` 必填、端口/探针默认按 runtime 镜像契约）+ Secret `<w>-cimicode-fs`（FS 凭据明文只读复制 + `model-config`：spec.model + bridge pod 网关 env 渲染 agentteams-gateway provider，native-config 哨兵跳过、`CIMICODE_MODEL_CONFIG_HASH` 明文哈希 env 变更触发滚动重启）+ 工作 env（WORKER_NAME/FS_*/TEAM/MATRIX_USER_ID——team 经 Team CR workerMembers 反查，MATRIX_USER_ID 以 status.matrixUserID 为权威）并 patch Worker env `BRIDGE_RUNTIME_*` 两键（残留 HELPER 键显式清除）；bridge pod 未起或模型链缺失时本轮零副作用推迟（旧栈保留，下一轮重试） | v3 §4.3 ✅（真实运行时改造 2026-09-13；统一形态 2026-09-16） |
| `cimicode-runtime/` | **内网 runtime 镜像构建上下文**（单容器：`cimicode serve` :4096 + taskflow/agentteams-sync/mc + skills 全套；coder-cimicode 基础镜像 `CIMICODE_BASE_IMAGE` 构建必填、仓库内无默认值——内网 registry 地址不进外网仓库；零凭据，模型经 `OPENCODE_CONFIG_CONTENT` 运行时注入） | 真实运行时 ✅（2026-09-13；统一形态重写 2026-09-16） |
| `opencode-runtime/` | **外网 runtime 镜像构建上下文**（与 cimicode-runtime 同契约：node:22-slim + npm `opencode-ai@1.18.27`，`opencode serve` :4096；apt 补 ripgrep/python3；共享 `bin/mc` + skills；零凭据同注入链——内外网切换只换镜像） | 统一形态 ✅（2026-09-16） |
| `bin/` | vendored mc 二进制（两 runtime Dockerfile 共享 COPY 源） | 共享资产 ✅ |
...
```bash
# 单测（共 208 个；开发机需 pip install pyyaml——bridge 生成工具做结构化解析）
...
cd bridge && python -m pytest tests -q        # 114 个（进程单测 71：adapter/system 字段/bootstrap/env/自愈轮询 + 生成器 43：golden×2 + coordination.go 文案对齐 + 解析器/Persona/CLI）
cd ../operator && python -m pytest tests -q    # 24 个（operator：单端口 svc/模型注入+secretKeyRef+哈希 drift/两键 patch/残留 HELPER 清除/GC/推迟供给）
