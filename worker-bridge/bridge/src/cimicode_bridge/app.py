...
session.HistoryManager，gateway 调用在 runtime（cimicode SSE，stateless/pod
两形态由 runtime/registry 工厂分派），Matrix 收发在 matrix/gateway，controller
交互（401 刷新 / 晚到接线自愈）在 controller/client——本模块只做装配与串联。
"""
...
class BridgeApp:
    ...
    runtime_client: Any | None = field(default=None, init=False)                  # runtime adapter（cimicode-stateless / cimicode-pod）
    ...
    # BRIDGE_RUNTIME_BASE_URL（或 controller runtimeEnv）显式提供过值时置
    # True——runtime.yaml bridge 段不得遮蔽显式 env 覆盖（排障用）。
    _explicit_base_url: bool = field(default=False, init=False, repr=False)
    # 已见证过的 adapter 形态（自愈轮询变更检测用）
    _last_adapter_mode: str = field(default="", init=False, repr=False)
    ...
    def start(self) -> None:
        ...
        # runtime.yaml 顶层 bridge 段是 worker-bridge 的统一绑定通道
        # （adapterMode/baseUrl/sessionId/sandboxId/templateId）；openclaw.json
        # 的 bridge.runtime 段是 legacy cimicode 形态，仅作兼容兜底。
        if self.worker_files is not None:
            self._apply_bridge_section(self.worker_files)
        # adapter 形态判定（唯一权威顺序，见 _resolve_adapter_mode）
        ...
        print(f"Bridge started with runtime adapter: {self.config.runtime.adapter or '(undetermined)'}")
    ...
    def _apply_env_overrides(self) -> None:
        ...
        固化进镜像的 bridge 配置保持 adapter 无关；controller 托管的
        bridge pod 经 Worker CR spec.env 收到接线（BRIDGE_RUNTIME_ADAPTER /
        _BASE_URL——pod 模式由 operator patch，stateless/排障可手动）。
        显式 env 优先于 runtime.yaml bridge 段（见 _apply_bridge_section）。
        """
    ...
    def _build_matrix_gateway(self) -> MatrixGateway | None:
        ...
        只有 cimicode-stateless 要求预创建的绑定（runtime.yaml bridge 段的
        sessionId/sandboxId，来自外部平台）；cimicode-pod 的会话由 pod 内
        cimicode 自管。晚到的接线（pod 创建后 Worker spec.env 才写入）由
    ...
    async def start_background(self) -> None:
        ...
        if self.matrix_gateway.user_id:
            ...
            self.mention_filter.role_resolver.self_user_id = self.matrix_gateway.user_id
        self.runtime_healthy = self.matrix_gateway.connected
        # session 门禁 adapter 化：只有 cimicode-stateless 要求预创建的绑定
        session_required = self.config.runtime.adapter == "cimicode-stateless"
        self.ready = self.matrix_connected and (bool(self.config.runtime.session_id) or not session_required)
        self.phase = "listening" if self.ready else "bootstrap"
    ...
    async def handle_matrix_message(
        ...
        try:
            # ---- agent.md 组装（worker-bridge 唯一路径：v2.4 生成器）----
