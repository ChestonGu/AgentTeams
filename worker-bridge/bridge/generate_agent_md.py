...
worker-bridge pod does not consume that document: this generator reproduces
...
Inputs:
  ...
  --template       source template (defaults to the repo copy at
                   template/worker-bridge-agent/AGENTS.md — the static
...
try:
    import yaml
except ImportError:  # the bridge image ships PyYAML; a bare host lacks it
    yaml = None
...
DEFAULT_TEMPLATE = os.path.join(
    ...
    "template", "worker-bridge-agent", "AGENTS.md")
...
def parse_runtime_config(text):
    ...
    if yaml is None:
        raise GenerateError(
            ...
            "(pip install pyyaml; the bridge image ships it)")
    try:
...
def main(argv=None):
    ...
    ap = argparse.ArgumentParser(
        description="Generate the worker-bridge agent.md from the source "
