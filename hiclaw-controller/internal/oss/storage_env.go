...
const (
	...
	// attempt (default 2s).
	envStorageConnectTimeoutSeconds = "HICLAW_STORAGE_CONNECT_TIMEOUT_SECONDS"
	...
	envStorageRetryWindowSeconds = "HICLAW_STORAGE_RETRY_WINDOW_SECONDS"
	// envStorageRetryBackoffMS is the initial pause between retries, doubled
	// per attempt up to the cap (default 500).
	envStorageRetryBackoffMS = "HICLAW_STORAGE_RETRY_BACKOFF_MS"
	// envStorageRetryBackoffMaxMS bounds the per-attempt pause (default 5000).
	envStorageRetryBackoffMaxMS = "HICLAW_STORAGE_RETRY_BACKOFF_MAX_MS"
	...
	envStorageSDKMaxRetries = "HICLAW_STORAGE_SDK_MAX_RETRIES"
	// envStorageProbeTimeoutSeconds bounds the reachability probe before the
	// config phase (default 30s, matching the retry window).
	envStorageProbeTimeoutSeconds = "HICLAW_STORAGE_PROBE_TIMEOUT_SECONDS"
)
