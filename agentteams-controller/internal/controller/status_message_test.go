...
func TestConciseStatusMessage_StorageOpError(t *testing.T) {
	...
	opErr := &oss.OpError{Op: "get", Key: "agentteams-config/packages/alice.zip", Cause: leaf}
	chain := fmt.Errorf("deploy package: %w", fmt.Errorf("package resolve/extract failed: %w", opErr))

	got := conciseStatusMessage(chain)
	want := "storage get agentteams-config/packages/alice.zip: dial tcp 10.0.0.1:9000: i/o timeout"
	if got != want {
...
func TestConciseStatusMessage_SkipsOpaqueLeaf(t *testing.T) {
	...
	mcErr := fmt.Errorf("mc cp agentteams/agentteams-storage/agentteams-config/packages/alice.zip: %w (stderr: mc: unable to prepare URL for copying)", leaf)
