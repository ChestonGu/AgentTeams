...
func (k *K8sBackend) Create(ctx context.Context, req CreateRequest) (*WorkerResult, error) {
	...
	if image == "" {
		...
		case req.Runtime == RuntimeCimiCodeBridge && k.config.CimiCodeBridgeWorkerImage != "":
			image = k.config.CimiCodeBridgeWorkerImage
...
func defaultRuntime(runtime string) string {
	...
	case RuntimeCimiCodeBridge:
		return RuntimeCimiCodeBridge
	default:
