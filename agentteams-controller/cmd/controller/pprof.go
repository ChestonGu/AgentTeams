...
// `-tags pprof` (see agentteams-controller/Dockerfile ENABLE_PPROF build arg);
// the default build gets the no-op stub in pprof_stub.go. The listen
// address is overridable via AGENTTEAMS_PPROF_ADDR (default 0.0.0.0:6060,
...
func maybeStartPprof(ctx context.Context) {
	...
	addr := os.Getenv("AGENTTEAMS_PPROF_ADDR")
	if addr == "" {
		addr = "0.0.0.0:6060"
	}
