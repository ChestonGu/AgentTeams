...
# Defensive log() fallback: hiclaw-env.sh provides log() via base.sh (Manager
# images) or its own fallback, but this script must keep working even when
# both are absent (controller/worker images) or hiclaw-env.sh fails to load —
# otherwise an mc failure below turns into a misleading "log: command not found".
