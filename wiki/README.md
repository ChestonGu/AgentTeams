...
| [cimicode-runtime-integration.md](cimicode-runtime-integration.md) | **新运行时接入方案**（opencode 魔改无状态 code agent） | 无状态架构解读（大脑/手脚分离、单连接事务、六需求源码改动）、桥兼任编排层的集成架构（映射持久化/沙箱生命周期）、风险清单（R1-R12 架构与集成级 + P1-P6 平台接入级）、改动清单与分阶段实施顺序 | cimicode 开发者 |
