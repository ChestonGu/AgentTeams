...
| F-09 | mc 子进程风暴（Step 4 分钟级主因） | 每次 OSS 操作 `exec` fork mc 子进程 + TLS 握手 + 无连接池；每成员每轮 10~20 次调用，本地 MinIO 200~400ms/次、云 OSS 400~800ms+/次 | minio-go SDK 存储驱动（`HICLAW_STORAGE_DRIVER=sdk` 默认）：HTTP 连接池复用 + 5s dial 超时 + 3 次有界重试；bench 实测每成员 config 阶段延迟降 **~5.8×**；`mc` 驱动保留可回滚，动态 STS 凭据两驱动均支持 | [95dcae1](https://github.com/agentscope-ai/HiClaw/commit/95dcae1) |
| F-10 | 存储不稳定导致分钟级硬等待 | 端点不可达时 mc 默认 30s dial 超时逐调用叠加（每成员 10~20 次）；SDK 默认 10×30s 重试 | `probeStorage` 前置 10s 可达性探测快速失败；mc 驱动网络类错误重试 1 次（1s 间隔）；SDK 驱动 5s dial + 3 次重试（最坏 ~15-25s/op） | [e620ba2](https://github.com/agentscope-ai/HiClaw/commit/e620ba2) |
